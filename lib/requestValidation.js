...
// history entries are the model's own prior replies (trusted, server-
// generated), not attacker-controlled input like a fresh `message` — so
// they get a much more generous cap. A tight cap here caused a real bug:
// once a single reply crossed 4000 characters (increasingly likely as the
// knowledge base grew richer, confirmed-tier IFU content), every later
// message in that conversation failed validation permanently, since the
// oversized entry never leaves `history` until the rep clears the chat.
// The overall MAX_REQUEST_BODY_SIZE cap below still bounds worst-case
// request size regardless of this higher per-entry limit.
export const MAX_HISTORY_MESSAGE_LENGTH = 20000;
...
export function validateChatRequest(body) {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    if (turn.content.length > MAX_HISTORY_MESSAGE_LENGTH) {
      return `history[${i}].content exceeds the ${MAX_HISTORY_MESSAGE_LENGTH}-character limit.`;
    }
