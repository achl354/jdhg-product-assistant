# Clavia — Chemo/Dialysis Chairs & Surgery Stretcher

**Content type: marketing/sales collateral (JDHG flyers), not a manufacturer IFU or ARTG/regulatory document.** This file merges 3 source PDFs, one per product below.

## Clavia FCD — Chemo & Dialysis Trolley Chair
*Source: CF - JDHG Flyer - Clavia FCD - JDHG120924.pdf*

Electric trolley chair for chemotherapy/dialysis, six pre-programmed positions (Welcome, Relax, Reverse Trendelenburg, Secured Trendelenburg, CPR, plus a sixth profile).

- Three-section mattress support platform (choice of two widths); 10cm dual-layer hospital-grade foam mattress with viscoelastic top layer; removable waterproof slate-grey PVC-coated jersey cover (wipe down or launder)
- Robust powder-coated steel frame, ABS plastic base cover
- Electrically operated backrest (manual CPR release), Trendelenburg, reverse Trendelenburg, and leg rest; two Linak column actuators for height; hand control for all electric functions
- Height-adjustable armrest with anatomically shaped PU cushion, swings up behind backrest for lateral-transfer access; detachable height-adjustable headrest pillow
- One removable 2-stage 2-hook IV pole; anti-static chain; all-swivel independently lockable twin castors
- **Safe Working Load: 250kg**

| Attribute | Value |
|---|---|
| Product code | CFBM-CLVE4-7FCD |
| Mattress platform | 70cm W x 200cm L |
| Overall width | 104cm |
| Overall length (chair / stretcher profile) | 145cm / 202cm |
| Height range (inc. mattress) | 59cm – 99cm |
| Trendelenburg / reverse Trendelenburg | 0–15° / 0–6° |
| Leg rest range | 0–60° |
| Twin castor Ø | 125mm |

## Clavia LSA Surgery — General Purpose Day Surgery Stretcher
*Source: CF - JDHG Flyer - Clavia LSA Surgery - JDHG101024.pdf*

Same base platform as the FCD (electric backrest/Trendelenburg/reverse Trendelenburg/leg rest/height), designed so the patient is never transferred — the procedure is performed directly on the chair. **Safe Working Load: 200kg.**

Additional surgery-specific features: detachable tapered head section integrated into backrest, accepting three specialised surgical head-rest options (Ophthalmology — concave cushion, CFBM-LCA-OPH; Stomatology — flat cushion, CFBM-LCA-PLA; Stomatology/ENT — non-articulated concave cushion, CFBM-LCA-STO); surgical accessory rails on both sides of backrest frame; foot pedal height control; solid side rails that drop down and slide sideways for improved surgical access; corner bumpers with accessory receptacles (foot end); folding push handles (head and foot end); central-locking swivel twin castors plus retractable 5th wheel.

| Attribute | Value |
|---|---|
| Product code | CFBM-CLVE4-7LCA |
| Mattress platform | 75cm W x 200cm L |
| Overall width | 90cm |
| Overall length (chair / stretcher profile) | 152cm / 213cm |
| Height range (inc. mattress) | 59cm – 99cm |
| Trendelenburg / reverse Trendelenburg | 0–15° / 0–6° |
| Twin castor Ø | 150mm |

## Clavia AMB7 FCD — Chemotherapy Bed/Chair (newer model)
*Source: JDHG Flyer - Clavia AMB7 FCD - JDHG121225.pdf*

Ambulatory care chair for dialysis/chemotherapy with 4 pre-programmed positions: Patient Welcome, Nursing, Ultra-Comfort (Zero Gravity), Emergency Trendelenburg.

- Electrically adjustable height, backrest, legrest, Trendelenburg (10°) and reverse Trendelenburg (15°)
- Multi-directional "Body Flex" armrests (ball-and-socket adjustment) for vascular access positioning, retractable behind backrest for transfers
- Two mattress widths (65cm / 75cm); removable mattress with buttonhole fixing for fast replacement
- "Soft Sound" technology reduces movement noise/vibration; large 125mm castors with integrated braking
- Ergonomic caregiver remote control; patient control satellite with LED lighting + USB charging port; tool-free disassembly for cleaning
- **Max patient weight 185kg; Safe Working Load 250kg**
- Medical Device Class 1, CE marked (EU Regulation 2017/745)

| Attribute | 65cm variant | 75cm variant |
|---|---|---|
| Overall width | 94cm | 104cm |
| Product weight | 97kg | 100kg |

Other specs (both widths): overall length chair/lying position 156cm/202cm; variable height (lying, no mattress) 47–85cm; backrest inclination 0–78°; legrest inclination 0–54°; mattress 10cm high-resilience foam (35kg/m³), Class 0, PVC-coated jersey cover; plastic chassis cover over epoxy steel structure; individual braking; double-roller 125mm castors.

## Contact
sales@jdhealthcare.com.au | 1300 791 404
