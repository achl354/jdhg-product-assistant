...
In the UI (`public/index.html`), each bot reply renders as one bordered "response card" — status badge and answer, then (only when present) a tinted "copy-ready customer wording" section with a one-click Copy button, then sources/related documents, then the feedback row — sections divided by hairline rules rather than loosely stacked elements, so the reply looks as structured as the underlying answer contract actually is. An earlier separate amber "internal caveat" box was removed after pilot feedback — any internal caveat now stays inline in the main answer text (which was always the field's design — "the full answer, including any caveats inline"), rather than a second box. See "Layout" below for the rest of the interface (desktop rail, welcome state, header/composer).

`customer_ready` (`lib/prompt.js` rule 7) defaults to a short bulleted list of brief one-sentence points rather than a dense paragraph — an optional one-sentence intro, then bullets, is the norm; a single short sentence or two is only used when there's genuinely just one point. It's also instructed to never cite the source mid-sentence (no "per the manufacturer's care guide..." woven into the message) — instead it ends with the source on its own final line, formatted exactly as `Reference: <document name>`, separated from the body by a blank line. The client (`renderCustomerReady` in `public/index.html`) detects that trailing line and renders it as a small muted italic footer with a subtle divider, distinct from the main message, rather than as a regular paragraph — falling back to plain rendering if a reply doesn't follow the format, so this never breaks on an unexpected shape.
...
**Nothing is invented.** No formal JDHG document-approval workflow, named approver, or external-use clearance was available for any source at ingestion time — so `approval_status`, `approver_owner`, and `external_use_permission` are `"needs_confirmation"` for every single entry, including the pre-existing HoverTech/TrenGuard sources. A test (`test/sourceProvenance.test.js`) enforces this stays true rather than silently regressing to a guessed value later. `secure_document_link` started the same way, but has since been partially populated — see "Document links" below.

## Document links

When a rep asks for the actual IFU/flyer/certificate behind an answer, the bot links to the real source document rather than describing it in prose only. This is entirely server-driven — **the model never generates a URL itself**:

1. The model cites document IDs as it always did, via the existing `sources`/`related_documents` fields — nothing new is asked of it there. `lib/prompt.js` (rule 9) additionally tells it to say a document is "linked below" rather than typing a URL inline when a rep asks for one, and that it doesn't control whether a link actually ends up attached.
2. `sources.meta.json`'s `secure_document_link` field holds, per knowledge file, either the string `"needs_confirmation"` (no verified link — the original/default state) or an array of `{ name, url }` objects — one per physical source document a knowledge file merges. This was populated on 2026-08-12 by searching the live SharePoint tenant for each source document's *current* canonical shared-library copy (never a personal-OneDrive or draft copy) and recording its real `webUrl`. 22 of 32 entries now have at least one real link; the rest stay `"needs_confirmation"` — either no canonical shared copy could be found, the correct document itself was ambiguous, or the source isn't a shareable file at all (the manifest, a Teams chat message, an INTERNAL USE ONLY comparison document). See the field's own updated description in `sources.meta.json`'s `$schema_notes` for the full list of exclusion reasons, and `test/sourceProvenance.test.js` for a test that pins which specific entries are expected to remain unresolved.
3. `lib/documentLinks.js`'s `attachDocumentLinks(data, sourcesMeta)` runs server-side, after the model's `provide_answer` tool call has already resolved (wired into `lib/chatStream.js` via an optional `options.attachDocumentLinks` hook, called from `server.js`). It looks up every doc ID in `sources` then `related_documents` (de-duplicated) against `sourcesMeta`, and adds a `document_links` field to the response — one entry per doc ID that actually has a resolved link. A doc ID with no link contributes nothing; there's never a placeholder or broken link shown.
4. The UI (`public/index.html`) renders `document_links` as a small list of real `<a>` tags under the response card's sources/related lines, and mirrors them next to each source in the desktop rail — both purely additive to layout already described in "Layout" below.

**A caveat worth knowing:** a `secure_document_link` points at whatever revision is currently live in SharePoint, which for a handful of HoverTech manuals (HoverMatt, HT-Air 2300, SitAssist Pro, EMS Evacuation HoverJack) is a newer revision than what the knowledge file's own prose was written from — SharePoint has moved on since the original knowledge-base ingestion pass. The link is still the right *document*, just not guaranteed to be word-for-word the same revision the bot's answer text reflects. `revision_version`/`document_date` on each entry states what the prose reflects; re-syncing the knowledge content itself to the newer revisions is a separate, not-yet-done task (see `MARKETING_RESYNC.md`'s general re-sync process).
...
The interface is a single, focused chat column — no permanent sidebar/quick-links — kept deliberately simple after early pilot feedback that a static topic-picker sidebar wasn't adding value. Four things build on that baseline:

- **Desktop canvas + answer-detail rail.** Below 1200px wide, this is the whole interface. At 1200px and above, a soft navy-tinted page canvas replaces the flat grey background, and a right-hand rail (`#rail` in `public/index.html`) mirrors the *latest* reply's sources and related documents as standalone chips — clicking a "related" chip fills the composer with a follow-up question (`fillComposer`), same as typing it. The rail is a convenience, never the only place this information lives: the response card always shows sources/related inline too, so nothing is lost with the rail hidden on narrower screens.
- **Fluid desktop width.** `#app`'s width is `clamp(680px, 55vw, 960px)` rather than a fixed pixel value (the rail is `clamp(260px, 20vw, 340px)` on the same idea) — the column actually grows on a wider monitor instead of staying pinned at 680px with dead canvas on either side, but still caps out at 960px so a line of chat text never gets unreasonably long on an ultrawide or 4K display. Mobile is unaffected: `clamp`'s 680px floor is still capped by `max-width: 100%` exactly as the old fixed width was, so nothing changes below the point where the column would need to shrink.
- **Unified response card.** Badge, answer, customer-ready wording, sources/related, and feedback are sections of one bordered card (`.response-card`) with hairline dividers, rather than separately spaced elements trailing below a chat bubble.
- **Welcome state.** Before the first message, the log shows a quiet welcome panel with the brand mark and four tappable example questions (`renderWelcome`) instead of a plain chat-bubble greeting — clicking one fills the composer, same as the rail's follow-up chips. It disappears the moment a real message is sent (`clearWelcomeIfPresent`) and never reappears for that conversation, including after a page refresh (a restored conversation with any history skips straight to replaying it).
- **Header.** Shows the full JD Healthcare Group logo (in white, directly on the navy background — see "Brand mark" below), with the tool's own name, "Sales Assistant" (also the browser tab title), and a "Pilot" pill to its right, generously spaced so the logo and text read as one composed lockup rather than a cramped cluster — an earlier version stacked "Sales Assistant" directly under the logo (mimicking the real logo's own "Group" sub-line) and packed everything tightly, but ended up feeling crowded; this settled on a wider horizontal layout instead, keeping "Sales Assistant" lighter-weight and smaller than a normal heading so it still reads as related to the logo rather than a bolted-on label (an italic treatment was tried and dropped — upright reads cleaner). No subtitle line — the earlier "Grounded only in the curated knowledge base" caption was removed from both the header and the composer, on the view that the welcome panel and the four-status model already communicate scope clearly enough without repeating it as chrome. "Clear" is an icon-only button (`aria-label`/`title` carry the accessible name); a status dot sits beside it. The background itself is a three-stop gradient (`--navy-dark` &rarr; `--navy` &rarr; `#2C5AA0`) rather than the original near-flat two-tone, reusing colors already in the palette rather than introducing a new one (`#2C5AA0` was originally the endpoint of the outgoing chat-bubble's gradient, before that bubble went flat — see below).
- **User message bubble.** Went through a few rounds of iteration: originally a navy-to-blue gradient (`linear-gradient(135deg, #1B3F73, #2C5AA0)`), then flat solid navy, then a cooler slate-grey (`#374151`), settling on a true neutral grey (`--bubble-user: #404040`, `R=G=B`) — a deliberate move away from the brand navy for the user's own messages, trading some brand presence for a calmer, more neutral feel. That grey needed a lighter dark-mode override (`#52525B`) — `#404040` barely contrasted against the dark theme's near-black panel/canvas, so it's one of the few tokens that isn't shared between light and dark (most colors in this file are; this one earns the exception because dark surfaces need a *lighter* neutral to read as a distinct bubble, not a darker one).
- **Adaptive mobile height.** `#shell`/`.app-frame` use `100dvh` (falling back to `100vh` on older browsers) instead of a plain `100vh`. Plain `100vh` on mobile Chrome/Safari is sized against the *largest* possible viewport (address bar hidden) — so whenever the address bar is actually showing, the real visible area is shorter than `100vh`, and the composer gets pushed down past the visible screen. `100dvh` tracks the real, current visible viewport instead.

### Brand mark

The header, welcome panel, and favicon use the real JD Healthcare Group mark instead of a placeholder "JD" text badge. Chat avatars deliberately stay plain "JD" text on a navy circle in *both* themes — a white-icon-on-navy treatment (and later a dark-mode-only version of it) was tried twice and reverted both times, settling on the simple monogram as reading cleaner than the logo at that small size regardless of theme. There's no source vector, so the image variants that remain are cropped/derived from the one supplied logo file; if the logo is ever updated, re-derive from the new file rather than hand-editing the PNGs.
...
`createBrandIcon()` in `public/index.html` is the single place the icon image path and alt text are set, used by the welcome panel. `createAvatarEl()` builds the plain-text "JD" avatar used in the three chat-avatar contexts (message, typing indicator, streaming preview). The header's logo `<img>` is separate markup (it's the full logo, not just the icon) but follows the same asset convention.

## Interface polish

- **Typeface.** Questrial, self-hosted (`public/assets/fonts/Questrial-Regular.woff2`, `@font-face` in `public/index.html`) rather than a live Google Fonts request — no external network dependency at runtime, consistent with the favicon/logo assets below. Falls back to the system font stack (`-apple-system`, Segoe UI, Roboto, etc.) while loading or if the file fails to fetch. One thing worth knowing: Questrial ships only a single Regular (400) weight — there's no true bold cut — so every `font-weight: 700`/`800` element (headings, buttons, badges) renders as the browser's synthesized ("faux") bold rather than a designed bold face. It reads fine in practice, but it's a real constraint of the font, not a bug, if it's ever revisited.
...
- **Dark mode.** Follows the OS/browser's `prefers-color-scheme` by default (colors are CSS custom properties, overridden as a block in a `@media (prefers-color-scheme: dark)` rule), plus a manual sun/moon toggle button in the header (`#themeToggle`) for reps who want a different setting than their OS. Clicking it sets an explicit `data-theme="light"` or `data-theme="dark"` attribute on `<html>`; two more variable blocks (`:root[data-theme="dark"]`, `:root[data-theme="light"]`) mirror the same values as the base/media-query blocks but win regardless of system preference, since an attribute selector always beats a plain `:root` selector. The choice is saved to `localStorage` (`jdhg-theme-preference`) and re-applied by a small inline script at the very top of `<head>` — before the stylesheet or any visible content — so a saved choice never flashes the system-default theme first on reload. Both icons (clean hand-drawn sun/moon, not emoji) are always in the DOM; which one is visible is pure CSS (`.sun-icon`/`.moon-icon` display rules keyed off the same three states — default, media query, explicit attribute) rather than JS swapping elements on click.
- **Favicon.** `public/assets/favicon.png`, cropped from the real JD Healthcare Group logo (see "Brand mark" below) — no external asset/network request.
...
- **Request validation** (`lib/requestValidation.js`, enforced before anything else in `/api/chat`): `message` (the rep's fresh input) must be a non-empty string up to 4,000 characters; `history` (if present) must be an array of up to 40 entries, each `{ role: "user"|"assistant", content: string }` with no extra fields, where `content` is capped at a much more generous 20,000 characters (`MAX_HISTORY_MESSAGE_LENGTH`) rather than the same 4,000-character cap as fresh input. History entries are the model's own prior replies — trusted, server-generated content, not attacker-controlled input — and a tight cap there caused a real bug: once a single reply crossed 4,000 characters, every later message in that conversation failed validation permanently, since the oversized entry never leaves `history` until the rep hits "Clear." The client (`public/index.html`) also defensively truncates what it stores in `history` (never what's shown on-screen) at the same 20,000-character mark, so this can't recur regardless of how long a future answer gets. Anything malformed returns HTTP 400 with a short, generic description — never echoing request content back or leaking internals.
...
- Document links (see "Document links" above) are a point-in-time export, the same as `knowledge/marketing/` — populated by a one-off SharePoint search pass (2026-08-12), not a live sync. A document moved/renamed in SharePoint afterwards will silently stop resolving (the link in `sources.meta.json` just goes stale) rather than being caught automatically; there's no periodic re-check.
...
No formal JDHG document-approval workflow, named approver/owner, or external-use (customer-facing) clearance exists in the systems this pilot had access to, for **any** source — including the pre-existing HoverTech, TrenGuard, and EasiMove/EasiAir IFUs/ARTG certificates. Concretely, `approval_status`, `approver_owner`, and `external_use_permission` are `needs_confirmation` for every entry in `knowledge/sources.meta.json`. Before this pilot is used to produce customer-facing wording at scale, JDHG/Regulatory should confirm, per source (or per document type as a shortcut): who is the approving owner, and whether it's cleared for external/customer use. (`secure_document_link` — where its current copy actually lives — is a separate, narrower gap: see "Document links" above; 22/32 sources now have a real link, the rest are listed there.)
...
7. **EasiMove ARTG number (`easimove-artg-and-regulatory.md`)** — **resolved for SPU**: JDHG confirmed 528531 as the ARTG number for EasiMove SPU (2026-07-29). Still open: whether 528531 also applies to EasiMove PRO specifically (the source file was originally saved as "EasiMove PRO ARTG," so this hasn't been separately re-confirmed), and what the older 343300 entry (registered 2020, different manufacturer, generic "Lift, &lt;specify&gt;" GMDN term) actually covers, if anything current. **Needs confirmation with JDHG/Regulatory** before quoting an ARTG number for EasiMove PRO or anything requiring absolute certainty (a tender or compliance document).
