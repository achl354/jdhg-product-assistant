...
- **Header.** Shows the full JD Healthcare Group logo (in white, directly on the navy background — see "Brand mark" below) next to the tool's own name, "Sales Assistant" (also the browser tab title), and a "Pilot" pill. No subtitle line — the earlier "Grounded only in the curated knowledge base" caption was removed from both the header and the composer, on the view that the welcome panel and the four-status model already communicate scope clearly enough without repeating it as chrome. "Clear" is an icon-only button (`aria-label`/`title` carry the accessible name); a status dot sits beside it.
