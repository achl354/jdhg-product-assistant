...
- **Header and composer.** A small "Pilot" pill and a status dot sit next to the title; "Clear" is now an icon-only button (`aria-label`/`title` carry the accessible name). A one-line caption under the composer states the scope up front: "Answers are grounded only in JDHG's curated knowledge base."
