...
- **Header.** Shows the full JD Healthcare Group logo (in white, directly on the navy background — see "Brand mark" below), with the tool's own name, "Sales Assistant" (also the browser tab title), directly beneath it as a small italic tagline — deliberately styled after the real logo's own "Group" sub-line (smaller, lighter, tucked under the main wordmark) so it reads as part of one continuous lockup rather than a separate UI label bolted next to the logo. The "Pilot" pill sits beside the tagline, staying visually distinct as a status badge rather than part of the brand block. No subtitle line — the earlier "Grounded only in the curated knowledge base" caption was removed from both the header and the composer, on the view that the welcome panel and the four-status model already communicate scope clearly enough without repeating it as chrome. "Clear" is an icon-only button (`aria-label`/`title` carry the accessible name); a status dot sits beside it.
